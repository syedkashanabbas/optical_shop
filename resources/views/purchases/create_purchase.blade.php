        ...
        <div class="card">
          <div class="card-body">
            <div class="row">
              ...
              <!-- Attachment Upload -->
            <div class="form-group col-md-4">
                  <label>{{ __('translate.Attachment') }}</label>
                  <input type="file" ref="attachment" @change="handleAttachment" class="form-control" accept=".pdf,.jpg,.png,.jpeg,.docx,.doc">
            </div>
...
<script>
  ...
  if (this.verifiedForm()) {
    ...
    formData.append("tax_rate", this.purchase.tax_rate || 0);
    formData.append("TaxNet", this.purchase.TaxNet || 0);
    formData.append("discount", this.purchase.discount || 0);
    formData.append("discount_type", this.purchase.discount_type || 'fixed');
    formData.append("discount_percent_total", this.purchase.discount_percent_total || 0);
    formData.append("shipping", this.purchase.shipping || 0);
    formData.append("GrandTotal", this.GrandTotal || 0);
    ...
    axios.post("/purchase/purchases", formData, {
      headers: {
    ...
    Get_Product_Details(product_id , variant_id) {
      ...
      mounted() {
  if (this.warehouses.length > 0 && !this.purchase.warehouse_id) {
    this.purchase.warehouse_id = this.warehouses[0].id;

    setTimeout(() => {
      this.Selected_Warehouse(this.purchase.warehouse_id);
    }, 300); // smooth UI, no lag
  }
}
,
      //-----------------------------Autoload function-------------------
