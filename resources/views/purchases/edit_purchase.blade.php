...
<div class="form-group col-md-4">
  <validation-provider name="Attachment" rules="mimes:pdf,jpg,jpeg,png,doc,docx,xlsx,xls" v-slot="{ errors }">
...
onFileChange(e) {
  this.purchase.attachment = e.target.files[0];
},
  ...
  if (this.verifiedForm()) {
    ...
    formData.append('discount_type', this.purchase.discount_type || '');
    ...
    if (this.purchase.attachment instanceof File) {
      formData.append('attachment', this.purchase.attachment);
    }

    axios.post(`/purchase/purchases/${this.purchase.id}?_method=PUT`, formData, {
      headers: {
