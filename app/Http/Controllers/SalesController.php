    ...
    public function get_sales_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('sales_view_all') && !$user_auth->can('sales_view_own')){
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('Ref', 'LIKE', "%{$request->input('search.value')}%")
                     ...
                     ->orWhere('assigned_driver', 'LIKE', "%{$request->input('search.value')}%");
            ...
            foreach ($sales as $sale) {
                ...
                $item['assigned_driver'] = $sale->assigned_driver ?? 'Not Assigned';
...
$previous_balance = $opening_balance + ($total_amount_prev - $total_paid_prev);
        ...
        ////// not taking client opening in account /////////
        $previous_balance = $opening_balance + DB::table('sales')
    ->where('deleted_at', null)
    ->where('client_id', $client_id)
    ->where('id', '!=', $sale_data->id)
    ->sum(DB::raw('GrandTotal - paid_amount')); // sum unpaid portion directly

$updated_balance = $previous_balance + ($sale_data->GrandTotal - $sale_data->paid_amount);
///////////////////////////////////////////////////////////////////////////////
        ...
        // dd($sale_data->toArray(), $total_amount_prev, $total_paid_prev);
        // dd([
        //     'sale' => $sale_details,
        //     'details' => $details,
        //     'company' => $company,
