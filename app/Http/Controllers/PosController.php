...
\App\Services\ClientLedgerService::log(
    ...
    $order->Ref,
    $order->GrandTotal,      // debit = sale amount
    0                        // credit = 0
);

// 2️⃣ POS payment entry (credit)
\App\Services\ClientLedgerService::log(
    $request->client_id,
    'pos_payment',           // type = POS payment
    $order->Ref,
    0,                       // debit = 0
    $order->GrandTotal       // credit = payment received
);
...
public function Print_Invoice_POS(Request $request, $id)
{
    ...
    if ($user_auth->can('pos')) {
        $details = array();
        $total_product_discount = 0; // Initialize product discount total

        $sale = Sale::with('details.product.unitSale')
            ->where('deleted_at', '=', null)
            ->findOrFail($id);

        // Calculate product discounts and prepare details
        foreach ($sale['details'] as $detail) {
            // Calculate discount amount for this product
            if ($detail->discount_method == '2') {
                $discount_amount = $detail->discount;
            } else {
                $discount_amount = $detail->price * $detail->discount / 100;
            }
            $total_product_discount += $discount_amount * $detail->quantity;

            // Prepare product details (existing code)
        ...
        $item['product_discount_total'] = $this->render_price_with_symbol_placement(number_format($total_product_discount, 2, '.', ','));
        ...
        $previous_balance = $opening_balance + ($total_amount - $total_paid);
        $previous_balance_formatted = $this->render_price_with_symbol_placement(number_format($previous_balance, 2, '.', ','));
