    ...
    if ($user_auth->can('adjustment_edit')) {
        \DB::transaction(function () use ($request, $id) {
            ...
            foreach ($old_adjustment_details as $key => $value) {
                ...
                $variant_id = !empty($value['product_variant_id']) ? $value['product_variant_id'] : null;
                ...
                \App\Services\LedgerService::log(
                    $value['product_id'],
                    'adjustment_edit_revert',
                    $current_adjustment->Ref,
                    $value['type'] === 'sub' ? $value['quantity'] : 0,
                    $value['type'] === 'add' ? $value['quantity'] : 0
                );
            ...
            foreach ($new_adjustment_details as $key => $product_detail) {
                $variant_id = !empty($product_detail['product_variant_id']) ? $product_detail['product_variant_id'] : null;
                ...
                \App\Services\LedgerService::log(
                    ...
                    'adjustment_edit',
                    $current_adjustment->Ref,
                    $product_detail['type'] === 'add' ? $product_detail['quantity'] : 0,
                    $product_detail['type'] === 'sub' ? $product_detail['quantity'] : 0
                );
