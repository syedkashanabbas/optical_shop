...
public function report_profit(Request $request)
{
    ...
    if ($user_auth->can('report_profit')){
        ...
        //------------ Calculate Total Discounts on Sales ------------\\
        ///////Old//////////////////////////
        // $sales_with_discounts = Sale::where('deleted_at', '=', null)
        //     ->whereDate('date', '>=', $start_date)
        //     ->whereDate('date', '<=', $end_date)
        //     ->where(function ($query) use ($request, $array_warehouses_id) {
        //         return $query->whereIn('warehouse_id', $array_warehouses_id);
        //     })
        //     ->select('discount', 'discount_type', 'discount_percent_total', 'GrandTotal')
        //     ->get();

        // $total_discounts = 0;
        // $discount_count = 0;

        // foreach ($sales_with_discounts as $sale) {
        //     if ($sale->discount > 0) {
        //         $discount_count++;
        //         if ($sale->discount_type == 'percent') {
        //             // Use the already calculated discount_percent_total
        //             $total_discounts += $sale->discount_percent_total;
        //         } else {
        //             // Fixed discount
        ...
        // $item['total_discounts'] = $this->render_price_with_symbol_placement(number_format($total_discounts, 2, '.', ','));
        // $item['discount_count'] = $discount_count;
...
// Load the sale with its details
...
foreach ($sales as $sale) {
    ...
    // ---- Detail-level discounts ----
    foreach ($sale->details as $detail) {
        // Only count if the sale-level discount doesn't already include details
        if ($detail->discount > 0 && $sale->discount_percent_total == 0) {
            $discount_count++;
            if ($detail->discount_method == '1') { // percent
                $total_discounts += ($detail->price * $detail->quantity) * ($detail->discount / 100);
            } else { // fixed
                $total_discounts += $detail->discount;
            }
        }
    }
}
