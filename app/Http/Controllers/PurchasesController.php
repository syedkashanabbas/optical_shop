...
//updated becasue the client wants it ito relfect in payemnts controller of purchases
        ...
        if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
            ...
            if ($user_auth->can('purchases_delete')){
                ...
                \DB::transaction(function () use ($id) {
                    ...
                    $original = \App\Models\ProviderLedger::where('reference', $current_Purchase->Ref)->first();
if ($original) {
    \App\Services\ProviderLedgerService::log(
        $original->provider_id,
        'purchase_deleted',
        $original->reference,
        $original->credit,  // flip debit/credit
        $original->debit
    );
}
